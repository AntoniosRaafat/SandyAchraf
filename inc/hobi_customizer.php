...
class hobi_customizer {
	...
	public function __construct($data) {
		$this->data = $data;
		add_action('customize_register',array($this,'hobi_customize_register'));
	...
	function custom_admin_style($hook) {
		if ( 'customize.php' == $hook || 'widgets.php' == $hook ) {
			wp_enqueue_style( 'hobi-control-admin-css', get_template_directory_uri() . '/inc/css/hobi-customizer.css',array());
			wp_enqueue_script( 'hobi-control-admin-js', get_template_directory_uri() . '/inc/js/hobi-customizer.js', array( 'jquery' ), '20150611', true );
		}
	}	
	public function hobi_customize_register($customizeObj) {
	...
	public function create_control($value,$key) {
		...
		if ( isset($value['type']) ) {
			...
			switch($value['type']){
				...
				case "switch":
					$args['type'] = $value['type'];
					$inst = new hobi_Customize_Switch_Control($this->customizeObj,$id,$args);
					break;
				case "image":
	...
	public static function hobi_data_load() {

		$data = apply_filters( 'hobi_customizer_data', array());
...
add_action('init','hobi_customizer::hobi_data_load');
...
if(class_exists( 'WP_Customize_control')) {
	...
	class hobi_Customize_Switch_Control extends WP_Customize_Control {
		public $type = 'switch';
		public function render_content() {
			...
			<label>
				...
				<div class="switch_options">
					<span class="switch_enable"><?php esc_html_e('Yes','hobi'); ?></span>
					<span class="switch_disable"><?php esc_html_e('No','hobi'); ?></span>  
